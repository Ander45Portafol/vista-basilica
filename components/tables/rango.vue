            ...
            <div class="flex justify-start w-3/4 items-center max-[400px]:w-full">
                ...
                <div
                    ...
                    <p class="font-extrabold text-xl text-salte-900 max-[750px]:text-[18px]">
                        {{ rango.campos.nombre_rango }}</p>
                    <p class="font-normal text-sm text-gray-500 max-[750px]:text-[12px]">
                        {{ rango.campos.cantidad_monetaria_minima }}
                    </p>
                </div>
            </div>
                    ...
                    <form action="" @submit.prevent="submitForm()" class="flex justify-evenly">
                        <div class="flex-col w-64">
                            ...
                            <div class="relative z-0">
                                <input type="text" id="nombre_rango" name="nombre_rango" v-model="form.nombre_rango" maxlength="100" required
                            ...
                            <div v-if="!validarNombreRango()" class="flex mt-2 mb-0 text-sm text-red-400 bg-transparent"
                                role="alert">
                                ...
                                <div>
                                    El nombre del rango solo permite caracteres <span class="font-medium">
                            ...
                            <div class="relative z-0 mt-10">
                                ...
                                <span class="text-xs text-gray-400 absolute bottom-0.5 right-0" v-else> 0 /1000</span>
                                <label for="username"
                            ...
                            <div class="flex-col mt-6">
                                ...
                                <div class="flex justify-start mt-2">
                                    <label class="relative inline-flex items-center mb-5 cursor-pointer">
                                        <input type="checkbox" value="" class="sr-only peer" id="visibilidad_rango"
                                            name="visibilidad_rango" v-model="form.visibilidad_rango">
                                        <div
                        ...
                        <div class="flex-col w-64">
                            ...
                            <div class="modal-buttons mt-24 flex justify-end items-end">
                                ...
                                <button class="h-10 w-10 rounded-lg flex justify-center items-center" id="btnModalAdd"
                                    type="submit" :disabled="!validarNombreRango()">
                                ...
                                <button class="h-10 w-10 rounded-lg flex justify-center items-center" type="submit"
                                    id="btnModalUpdate" :disabled="!validarNombreRango()">
                                    <svg width=" 22px" height="22px" stroke-width="2" viewBox="0 0 24 24" fill="none"
                                        xmlns="http://www.w3.org/2000/svg" color="#000000">
                                        <path
                                            d="M3 19V5a2 2 0 012-2h11.172a2 2 0 011.414.586l2.828 2.828A2 2 0 0121 7.828V19a2 2 0 01-2 2H5a2 2 0 01-2-2z"
...
var api_url = "http://localhost:8000/storage/rangos/images/";
...
const cambiarImagen = () => {
    ...
    if (file && file[0]) {
        form.value.imagen_rango = file[0];
        console.log(form.value.imagen_rango);
...
//Metodo para agregar un nuevo rango
async function crearRango() {
    //Se actualiza el valor del token (esto para evitar errores con todos los refresh del token)
    token.value = localStorage.getItem('token');
    if (validarNombreRango() && validarDescripcionRango()) {
        try {
            ...
            FORMDATA.append("visibilidad_rango", form.value.visibilidad_rango ? 1 : 0);
            FORMDATA.append("imagen_rango", form.value.imagen_rango);
            ...
            TOAST.fire({
                icon: 'success',
                title: 'Rango creado exitosamente'
            });
...
async function leerUnRango(id) {
    //Se actualiza el valor del token (esto para evitar errores con todos los refresh del token)
    token.value = localStorage.getItem('token');
    try {
        await axios.get('/rangos/' + id, {
            headers: {
                Authorization: `Bearer ${token.value}`,
            },
        }).then(res => {
            ...
            form.value = {
                id_rango: res.data.data.id,
                nombre_rango: res.data.data.campos.nombre_rango,
                descripcion_rango: res.data.data.campos.descripcion_rango,
                cantidad_monetaria_minima: res.data.data.campos.cantidad_monetaria_minima,
                //Se convierte a true o false en caso de que devuelva 1 o 0, esto por que el input solo acepta true y false
                visibilidad_rango: res.data.data.campos.visibilidad_rango ? true : false
            };
            if (res.data.data.campos.imagen_rango != null) {
                form.value.imagen_rango = res.data.data.campos.imagen_rango;
                imagenPreview.value = api_url + form.value.imagen_rango;
            } else {
                form.value.imagen_rango = "";
            }
            //Se reinicia el timer
...
async function actualizarRango() {
    ...
    if (validarNombreRango() && validarDescripcionRango()) {
        try {
            ...
            await axios.post("/rangos_update/" + id, FORMDATA, {
                headers: {
                    "Content-Type": "multipart/form-data",
                    Authorization: `Bearer ${token.value}`,
                }
            ...
            TOAST.fire({
                icon: 'success',
                title: 'Rango actualizado exitosamente'
            });
        }
        catch (error) {
            console.log(error);
...
async function recuperarUnRango(id) {
...
function validarNombreRango() {
    var res = validaciones.validarSoloLetras(form.value.nombre_rango);
    return res;
}

function validarDescripcionRango() {
    var res = validaciones.validarSoloLetrasYNumeros(form.value.descripcion_rango);
    return res;
}
