<template>
  ...
  <div
    ...
    v-for="pagina in datos_paginas[paginacion - 1]"
    :key="pagina.id"
  >
    <div
    ...
    >
        ...
        >
          ...
          <p
            v-if="
              ...
              pagina.campos.descripcion_pagina != 'null'
            "
      ...
      >
        <button
          ...
          id="btnedit"
          v-if="pagina.campos.visibilidad_pagina == 1"
          @click.prevent="estadoActualizar(pagina.id)"
        >
          <svg
            width="26px"
            height="26px"
        ...
        <button
          @click="recuperarPagina(pagina.id)"
            ...
            <div class="flex-col w-72">
              ...
              <div class="relative z-0 mt-6">
                ...
                <input
                  ...
                  id="nombre_pagina"
                  name="nombre_pagina"
                  v-model="form.nombre_pagina"
                  @input="validarNombrePagina()"
                  maxlength="100"
                  required
              ...
              <div class="relative z-0 mt-6">
                ...
                <input
                  ...
                  id="numero_pagina"
                  name="numero_pagina"
                  v-model="form.numero_pagina"
                  @input="validarNumeroPagina()"
                  min="1"
                  max="10"
                  required
                  class="block py-2.5 px-0 w-full text-sm text-gray-200 bg-transparent border-0 border-b-2 border-gray-200 appearance-none focus:outline-none focus:ring-0 peer focus:border-moradoClaroLogin peer"
                ...
                <label
                  for="numero_pagina"
                  class="absolute text-sm text-gray-200 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                  >Número - Página <span class="text-sm ml-1"> * </span></label
                >
                <!-- Se coloca un if que evalua si la función de validar es false, así se muestra la alerta solo cuando es false -->
                <div
                  v-if="!validarNumeroPagina()"
                  class="flex mt-2 mb-0 text-sm text-red-400 bg-transparent"
                  role="alert"
                >
                  <svg
              ...
              <div class="relative z-0 mt-6">
                <textarea
                  id="descripcion_pagina"
                  name="descripcion_pagina"
                  maxlength="250"
                ...
                >
                <label
                  for="descripcion_pagina"
                  class="absolute text-sm text-gray-200 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                  >Descripcion - Página</label
                >
              </div>
              <div class="flex-col mt-6">
                <label for="visibilidad_pagina" class="text-sm text-gray-200"
                  >Visibilidad - Página <span class="text-sm ml-1"> * </span></label
                >
                <div class="flex justify-start mt-2">
                  <label class="relative inline-flex items-center mb-5 cursor-pointer">
                    <input
                      ...
                      value=""
                      class="sr-only peer"
                      id="visibilidad_pagina"
                      name="visibilidad_pagina"
                      v-model="form.visibilidad_pagina"
                    />
                    <div
...
async function crearPagina() {
  ...
  if (validarNumeroPagina() && validarNombrePagina()) {
    try {
      ...
      FORM_DATA.append("nombre_pagina", form.value.nombre_pagina);
      FORM_DATA.append("numero_pagina", form.value.numero_pagina);
      FORM_DATA.append("descripcion_pagina", form.value.descripcion_pagina);
      FORM_DATA.append("visibilidad_pagina", form.value.visibilidad_pagina ? 1 : 0);
      //Se realiza la petición axios mandando la ruta y el formData
      await axios
        .post("/paginas/", FORM_DATA, {
          headers: {
...
async function actualizarPagina() {
  ...
  if (validarNumeroPagina() && validarNombrePagina()) {
    try {
      ...
      //Se realiza la petición axios mandando la ruta y el formData
      await axios
        .post("/paginas_update/" + id, FORM_DATA, {
          headers: {
            Authorization: `Bearer ${token.value}`,
          },
        })
...
//Codigo para cambiar el estado de la pagina a inactivo
