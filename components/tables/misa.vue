<template>
  ...
  <div
    ...
    v-for="misa in datos_misas[paginacion - 1]"
    :key="misa.id"
  >
    <div
    ...
    >
      <div class="flex justify-start w-3/4 items-center max-[400px]:w-full">
        ...
        >
          ...
          <p class="font-extrabold text-xl text-salte-900 max-[750px]:text-[18px]">
            {{ misa.campos.titulo_misa }}
          </p>
          <p class="font-normal text-sm text-gray-500 max-[750px]:text-[12px]">
            {{ misa.campos.fecha_misa }}
          </p>
          <p class="font-normal text-sm text-gray-500 max-[750px]:text-[12px]">
            {{ misa.campos.descripcion_misa }}
          </p>
        </div>
      </div>
      ...
      >
        <button
          v-if="misa.campos.visibilidad_misa == 1"
          @click.prevent="estadoActualizar(misa.id)"
        ...
        <button
          @click="recuperarUnaMisa(misa.id)"
            ...
            <div class="flex-col w-64">
              ...
              <div class="relative z-0">
                ...
                <label
                  ...
                  >Enlace - Misa<span class="text-sm ml-1"> * </span></label
                >
              </div>
              <div class="relative z-0 mt-6">
                ...
                <label
                  for="fecha-misa"
                  class="absolute text-sm text-gray-200 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                  >Fecha - Misa<span class="text-sm ml-1"> * </span></label
                >
              </div>
              <div class="relative z-0 mt-6">
                <input
                  ...
                  id="titulo_misa"
                  name="titulo_misa"
                  v-model="form.titulo_misa"
                  required
                  @input="validarTituloMisa()"
                  maxlength="150"
                  class="block py-2.5 px-0 w-full text-sm text-gray-200 bg-transparent border-0 border-b-2 border-gray-200 appearance-none focus:outline-none focus:ring-0 peer focus:border-moradoClaroLogin peer"
                  placeholder=" "
                  autocomplete="off"
                ...
                <label
                  for="titulo-misa"
                  class="absolute text-sm text-gray-200 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                  >Título - Misa<span class="text-sm ml-1"> * </span></label
                >
              </div>
              <div
                v-if="!validarTituloMisa()"
              ...
              <div class="relative z-0 mt-6">
                <textarea
                  id="descripcion_misa"
                  name="descripcion_misa"
                  v-model="form.descripcion_misa"
                  maxlength="1000"
                ...
                >
                <label
                  for="descripcion-misa"
                  class="absolute text-sm text-gray-200 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                  >Descripción - Misa</label
                >
              </div>
              <div class="flex-col mt-6">
                <label for="visibilidad-misa" class="text-gray-200"
                  >Visibilidad - Misa</label
                >
                <div class="flex justify-start mt-2">
                  <label class="relative inline-flex items-center mb-5 cursor-pointer">
                    <input
                      ...
                      type="checkbox"
                      value="1"
                      name="visibilidad_misa"
                      v-model="form.visibilidad_misa"
                      class="sr-only peer"
                    />
                    <div
...
async function crearMisa() {
  if (validarTituloMisa()) {
    ...
    try {
      ...
      FORM_DATA.append("visibilidad_misa", form.value.visibilidad_misa ? 1 : 0);
      //Se realiza la petición axios mandando la ruta y el formData
      await axios
        .post("/misas/", FORM_DATA, {
          headers: {
...
async function actualizarMisa() {
  ...
  if (validarTituloMisa()) {
    try {
      ...
      //Se realiza la petición axios mandando la ruta y el formData
      await axios
        .post("/misas_update/" + id, FORM_DATA, {
          headers: {
            Authorization: `Bearer ${token.value}`,
          },
        })
