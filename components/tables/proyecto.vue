<template>
  <div
    ...
    v-for="proyecto in datos_proyectos[paginacion - 1]"
    :key="proyecto.id"
  >
    <div
    ...
    >
      <div class="flex justify-start w-3/4 items-center max-[400px]:w-full">
        <img
          :src="iconos_url + proyecto.campos.icono_proyecto"
        ...
        >
          <p class="font-extrabold text-xl text-salte-900 max-[750px]:text-[18px]">
            {{ proyecto.campos.nombre_proyecto }}
          </p>
          ...
          <p class="font-bold text-sm text-gray-500 max-[750px]:text-[12px]">
            ...
            <span class="font-normal text-sm text-gray-500 max-[750px]:text-[12px]">{{
              proyecto.campos.estado_proyecto
            }}</span>
          </p>
        </div>
      </div>
      <div
        class="buttons-data flex justify-center flex-row items-center max-[750px]:flex-col max-[400px]:flex-row max-[400px]:m-auto max-[400px]:mt-2 max-[400px]:flex-wrap"
      ...
      >
        <div class="flex max-[400px]:mr-2">
          ...
          <button
            ...
            v-if="proyecto.campos.visibilidad_proyecto == 1"
            @click="generarReporteProyecto(proyecto.id)"
          >
            <svg
              width="26px"
              height="26px"
        ...
        <div class="flex max-[400px]:mr-2">
          <button
            ...
            @click="estadoActualizar(proyecto.id)"
            v-if="proyecto.campos.visibilidad_proyecto == 1"
          >
            <svg
              width="26px"
          ...
          <form @submit.prevent="submitForm()" class="flex-col">
            <div class="flex justify-evenly w-full flex-wrap">
              <div class="flex-col w-64">
                ...
                <div class="relative z-0">
                  ...
                  <label
                    ...
                    >Nombre - Proyecto<span class="text-sm ml-1"> * </span></label
                  >
                </div>
                <div
                ...
                <div class="relative z-0 mt-6">
                  ...
                  />
                  <span
                    class="text-xs text-gray-400 absolute bottom-0.5 right-5"
                  ...
                  >
                  <label
                    for="descripcion_proyecto"
                    class="absolute text-sm text-gray-200 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                    >Descripción - Proyecto<span class="text-sm ml-1"> * </span></label
                  >
                </div>
                <div class="relative z-0 mt-6">
                  <label for="" class="text-gray-200 text-sm"
                    >Visibilidad - Proyecto</label
                  >
                  <div class="flex justify-start mt-2">
                    <label class="relative inline-flex items-center mb-5 cursor-pointer">
                      <input
                        ...
                        value=""
                        class="sr-only peer"
                        id="visibilidad_proyecto "
                        name="visibilidad_proyecto "
                        v-model="form.visibilidad_proyecto"
                      />
                      <div
              ...
              <div class="flex-col w-64">
                <div class="relative z-0">
                  ...
                  <label
                    for="meta_monetaria"
                    class="absolute text-sm text-gray-200 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                    >Meta - Monetaria<span class="text-sm ml-1"> * </span></label
                  >
                </div>
                <div class="pt-4 mt-2 flex-col">
                  <label for="" class="absolute text-gray-200 text-sm"
                    >Estado - Proyecto</label
                  >
                  <select
                    id="underline_select"
                    v-model="form.estado_proyecto"
              ...
              <div class="h-42 ml-32 mt-6 progressbar rounded-xl text-center min-w-[75%]">
                <p class="mb-4 mt-2 text-base text-white">
                  Porcentaje de logro actual registrado
                </p>
                <div class="flex items-center justify-center text-white">
                  <p class="mr-4">$0.00</p>
                  <div
                    class="w-2/4 bg-gray-200 h-12 rounded-2xl dark:bg-gray-700"
                    data-tooltip-target="tooltip-porcentaje"
                    data-tooltip-placement="bottom"
                  >
                    <div
                      v-if="formPorcentaje.porcentaje_logro > 0"
                      ...
                      :style="{ width: formPorcentaje.porcentaje_logro + '%' }"
                    ></div>
                  </div>
                  <p class="ml-4">${{ formPorcentaje.meta_monetaria_porcentaje }}</p>
                </div>
                <div
                  id="tooltip-porcentaje"
                  role="tooltip"
                ...
                <p class="text-white text-base mt-2">
                  {{ formPorcentaje.porcentaje_logro }}%
                </p>
              </div>
              <div class="flex flex-wrap w-64 mt-12 mb-24">
                <div class="flex ml-[-100px]">
                  <div class="flex-row">
                    ...
                    <div
                      class="ml-16 h-32 w-32 border-2 border-slate-900 rounded-lg cursor-pointer relative"
                      @click="seleccionarArchivoIcono"
                      @mouseover="trueIconoBorrarIcono"
                    ...
                    >
                      ...
                      />
                      <div
                        v-if="mostrarIconoBorrarIcono && iconoPreview"
                  ...
                  <div class="flex-col">
                    ...
                    >
                      <img
                        v-if="imagenPrincipalPreview"
                        :src="imagenPrincipalPreview"
                        class="h-32 w-32 rounded-lg"
                      />
                      <input
                        type="file"
                        ref="inputImagenPrincipal"
                        class="hidden"
                        @change="cambiarImagenPrincipal"
                      />
                      <div
                        v-if="mostrarIconoBorrarImagenPrincipal && imagenPrincipalPreview"
                        class="absolute inset-0 h-32 flex items-center justify-center bg-black bg-opacity-50 rounded-lg"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                ...
                <div
                  class="modal-buttons flex justify-end items-end absolute bottom-6 right-6"
                >
                  <button
        ...
        <div class="flex items-start justify-between p-4 rounded-t">
          <div class="flex-col ml-4 pt-4">
            ...
            <p class="text-lg font-medium text-gray-400">
              Imagenes referentes a proyectos
            </p>
          </div>
          <button
            type="button"
        ...
        <div class="p-8 space-y-6 flex-col">
          <div class="flex justify-between items-center">
            <p class="text-gray-100">
              Ingrese las imagenes que desee mostrar para sus proyectos publicos
            </p>
            <button
              class="bg-purpleLogin hover:bg-lightPurpleLogin h-10 w-14 flex justify-center items-center rounded-lg"
          ...
          <div class="h-80 w-full image-container grid grid-cols-5 gap-5 py-5 px-5">
            <div
              v-if="data_imagenes"
              v-for="item in data_imagenes"
              :key="item.id_imagen_proyecto"
              class="relative"
            >
              <img
                :src="imagenes_url + item.archivo_imagen"
                class="h-32 w-32 rounded-lg"
              />
              <div
                @click="agregarImagenBorrada(item.id_imagen_proyecto)"
            ...
            <div v-for="(imagen, id) in imagenesPreviewArray" :key="id" class="relative">
              ...
              <div
                @click="agregarImagenPreviewBorrada(id)"
                class="absolute inset-0 w-32 h-32 flex items-center justify-center rounded-lg opacity-0 hover:opacity-100"
                style="background-color: rgba(0, 0, 0, 0.5)"
              >
                <svg
          ...
          <div class="flex justify-between items-center mt-6">
            ...
            <button
              class="w-32 h-10 bg-space text-white mt-4 rounded-lg"
    ...
    await axios
      ...
      .then((res) => {
        ...
        if (res.data.porcentaje_proyecto) {
          formPorcentaje.value.cantidad_total =
            res.data.porcentaje_proyecto.cantidad_total;
          if (res.data.porcentaje_proyecto.porcentaje_logro <= 100) {
            formPorcentaje.value.porcentaje_logro =
              res.data.porcentaje_proyecto.porcentaje_logro;
          } else {
            formPorcentaje.value.porcentaje_logro = 100;
          }
          formPorcentaje.value.meta_monetaria_porcentaje =
            res.data.porcentaje_proyecto.meta_monetaria;
        } else {
          formPorcentaje.value.cantidad_total = 0.0;
          formPorcentaje.value.porcentaje_logro = 0.0;
          formPorcentaje.value.meta_monetaria_porcentaje =
            res.data.data.campos.meta_monetaria;
        }
...
async function actualizarProyecto() {
  if (validarNombreProyecto() && form.value.estado_proyecto != 0) {
    ...
    try {
      ...
      await axios
        .post("/proyectos_update/" + id, FORMDATA, {
          headers: {
            "Content-Type": "multipart/form-data",
            Authorization: `Bearer ${token.value}`,
          },
...
function agregarImagenBorrada(id) {
  ...
  data_imagenes.value = data_imagenes.value.filter(
    (item) => item.id_imagen_proyecto != id
  );
}
