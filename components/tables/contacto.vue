...
<template>
  ...
  <div
    ...
    v-for="contacto in datos_contactos[paginacion - 1]"
    :key="contacto.id"
  >
    <div
...
<<<<<<< Updated upstream
            ...
            <div
                ...
                <button class="h-10 w-10 rounded-md flex items-center justify-center reportbtn"
                    v-if="contacto.campos.visibilidad_contacto == 1" @click="generarReporteContacto(contacto.id)">
                            ...
                            <div class="flex-col mt-6">
                                ...
                                <div class="flex justify-start mt-2">
                                    <label class="relative inline-flex items-center mb-5 cursor-pointer">
                                        <input type="checkbox" value="" class="sr-only peer" id="visibilidad_contacto"
                                            name="visibilidad_contacto" v-model="form.visibilidad_contacto">
                                        <div
...
=======
        ...
        >
          <!--Con la implementación de una variable que permite visualizar la información contenida en cada uno-->
          <p class="font-extrabold text-xl text-salte-900 max-[750px]:text-[18px]">
            {{ contacto.campos.nombre_contacto }}
          </p>
          <p class="font-normal text-sm text-gray-500 max-[750px]:text-[12px]">
            {{ contacto.campos.correo_contacto }}
          </p>
>>>>>>> Stashed changes
        </div>
      </div>
      ...
      >
        <button
          ...
          @click="generarReporteContacto(contacto.id)"
        >
          <svg
            width="26px"
            height="26px"
        ...
        <button
          v-if="contacto.campos.visibilidad_contacto == 1"
          @click.prevent="estadoActualizar(contacto.id)"
          class="h-10 w-10 rounded-md flex items-center ml-4 justify-center editbtn max-[400px]:mx-4"
        >
          <svg
            width="26px"
        ...
        <button
          @click="recuperarUnContacto(contacto.id)"
          class="h-10 w-10 rounded-md flex items-center justify-center ml-4 changebtn max-[750px]:ml-0 max-[750px]:mt-2 max-[400px]:mt-0 max-[400px]:mx-4"
          v-else
        >
          <svg
            ...
            <div class="flex-col w-72">
              ...
              <div class="relative z-0 mt-6">
                ...
                <span
                  ...
                  v-if="form.nombre_contacto"
                >
                  {{ form.nombre_contacto.length }} /100
                </span>
                ...
                <label
                  ...
                  >Nombre - Contacto<span class="text-sm ml-1"> * </span></label
                >
              </div>
              <div
                v-if="!validarNombreContacto()"
              ...
              >
                ...
                <div>
                  El nombre del contacto solo permite caracteres
              ...
              <div class="relative z-0 mt-6">
                ...
                <span
                  class="text-xs text-gray-400 absolute bottom-0.5 right-0"
                  v-if="form.correo_contacto"
                >
                  {{ form.correo_contacto.length }} /150</span
                >
                <span class="text-xs text-gray-400 absolute bottom-0.5 right-0" v-else
                  >0 /150</span
                >
                <label
                  for="correo_contacto"
                  class="absolute text-sm text-gray-200 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                  >Correo - Contacto<span class="text-sm ml-1"> * </span></label
                >
              </div>
              <!-- Campo de entrada Tipo - Contacto -->
              <div class="pt-4 mt-4 flex-col">
                <label for="" class="absolute text-sm text-gray-200"
                  >Tipo - Contacto<span class="text-sm ml-1"> * </span></label
                >
                <select
                  id="tipo_contacto"
                  name="tipo_contacto"
                ...
                <div class="flex justify-start mt-2">
                  <label class="relative inline-flex items-center mb-5 cursor-pointer">
                    <input
                      ...
                      value=""
                      class="sr-only peer"
                      id="visibilidad_contacto"
                      name="visibilidad_contacto"
                      v-model="form.visibilidad_contacto"
                    />
                    <div
                      class="w-9 h-5 bg-gray-600 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"
                    ></div>
...
async function generarReporteContacto(id_contacto) {
  ...
  const ruta = new URL(`http://127.0.0.1:8000/api/contacto_reporte/` + id_contacto);
...
async function crearContacto() {
  if (form.value.tipo_contacto != 0 && validarNombreContacto()) {
    ...
    try {
      ...
      FORM_DATA.append("visibilidad_contacto", form.value.visibilidad_contacto ? 1 : 0);
      //Se realiza la petición axios mandando la ruta y el formData
      await axios
        .post("/contactos/", FORM_DATA, {
          headers: {
...
async function actualizarContacto() {
  ...
  if (form.value.tipo_contacto != 0 && validarNombreContacto()) {
    try {
      ...
      //Se realiza la petición axios mandando la ruta y el formData
      await axios
        .post("/contactos_update/" + id, FORM_DATA, {
          headers: {
            Authorization: `Bearer ${token.value}`,
          },
        })
