    ...
    <div
      ...
      <div class="flex justify-start w-3/4 items-center max-[400px]:w-full">
        ...
        <div class="datainfo flex-col ml-8 max-[400px]:p-0 max-[400px]:w-full max-[400px]:ml-0 max-[400px]:text-center">
          ...
          <p v-if="seccione.campos.ubicacion_seccion" class="font-semibold text-sm text-gray-500 max-[750px]:text-[12px]">
            Ubicacion de la seccion: {{ seccione.campos.ubicacion_seccion }}
          </p>
          <p v-if="seccione.campos.subtitulo_seccion && seccione.campos.subtitulo_seccion != 'null'" class="font-normal text-sm text-gray-500 max-[750px]:text-[12px]">
            {{ seccione.campos.subtitulo_seccion }}
          </p>
          <p v-else class="font-normal text-sm text-gray-500 max-[750px]:text-[12px]">
            Esta sección no tiene subtitulo
          </p>
          <p v-if="seccione.campos.descripcion_seccion && seccione.campos.descripcion_seccion != 'null'" class="font-normal text-sm text-gray-500 max-[750px]:text-[12px]">
            {{ seccione.campos.descripcion_seccion }}
          </p>
          <p v-else class="font-normal text-sm text-gray-500 max-[750px]:text-[12px]">
            Esta sección no tiene una descripción
          </p>
        </div>
      </div>
      ...
      <div class="relative rounded-lg shadow modal">
        ...
        <div class="flex items-start justify-between p-4 rounded-t">
          <div class="flex-col ml-4 pt-4">
            ...
            <p class="text-base font-medium text-gray-400">Sección</p>
          </div>
            ...
            <div class="flex-col w-64">
              ...
              <div class="relative z-0">
                ...
                <span class="text-xs text-gray-400 absolute bottom-0.5 right-0">
                  {{ form.titulo_seccion.length }} /100</span>
                ...
                <div v-if="!validarTituloSeccion()" class="flex mt-2 mb-0 text-sm text-red-400 bg-transparent"
                  role="alert">
              ...
              <div class="relative z-0 mt-6">
                ...
                <span class="text-xs text-gray-400 absolute bottom-0.5 right-0" v-if="form.subtitulo_seccion">
                  {{ form.subtitulo_seccion.length }} /100</span>
                ...
                <label for="subtitulo_seccion"
                  ...
                  - Seccion</label>
                <!-- Se coloca un if que evalua si la función de validar es false, así se muestra la alerta solo cuando es false -->
                <div v-if="!validarSubtituloSeccion()" class="flex mt-2 mb-0 text-sm text-red-400 bg-transparent"
                  role="alert">
                  <svg aria-hidden="true" class="flex-shrink-0 inline w-5 h-5 mr-3" fill="currentColor"
                    viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
                  ...
                  <div>
                    El subtitulo de la sección solo permite caracteres
              ...
              <div class="relative z-0 mt-6">
                <textarea type="text" v-model="form.descripcion_seccion" id="descripcion_seccion" maxlength="250"
                  ...
                  placeholder=" " autocomplete="off" />
                <span class="text-xs text-gray-400 absolute bottom-0.5 right-5" v-if="form.descripcion_seccion">
              ...
              <div class="relative z-0 mt-6">
                <input type="number" v-model="form.ubicacion_seccion" id="ubicacion_seccion" min="1" max="99"
                  maxlength="250" name="ubicacion_seccion"
                  class="block py-2.5 min-h-[3rem] h-[3rem] max-h-[9rem] px-0 w-full text-sm text-gray-200 bg-transparent border-0 border-b-2 border-gray-200 appearance-none focus:outline-none focus:ring-0 peer focus:border-moradoClaroLogin peer"
                  placeholder=" " autocomplete="off" required />
              ...
              <div class="pt-4 mt-4 flex-col">
                ...
                <div v-if="form.id_pagina == 0" class="flex mt-2 mb-0 text-sm text-red-400 bg-transparent" role="alert">
                  <svg aria-hidden="true" class="flex-shrink-0 inline w-5 h-5 mr-3" fill="currentColor"
                    viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
                      d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
              ...
              <div class="flex-col mt-6">
                ...
                <div class="flex justify-start mt-2">
                  <label class="relative inline-flex items-center mb-5 cursor-pointer">
                    <input type="checkbox" value="false" v-model="form.visibilidad_seccion" class="sr-only peer" />
                    <div
              ...
              <div class="modal-buttons mt-6 flex justify-end items-end">
                ...
                <button id="btnModalAdd" type="submit" :disabled="!validarTituloSeccion() ||
                  !validarSubtituloSeccion() ||
                  form.id_pagina == 0
                ...
                <button id="btnModalUpdate" type="submit" :disabled="!validarTituloSeccion() ||
                  !validarSubtituloSeccion() ||
                  form.id_pagina == 0
                  " class="h-10 ml-2 w-10 rounded-lg flex justify-center items-center">
                  <svg width="22px" height="22px" stroke-width="2" viewBox="0 0 24 24" fill="none"
...
async function crearSeccion() {
  ...
  if (validarTituloSeccion() && validarSubtituloSeccion() && form.value.id_pagina != 0) {
    try {
      ...
      FORM_DATA.append("visibilidad_seccion", form.value.visibilidad_seccion ? 1 : 0);
      FORM_DATA.append("editable", form.value.editable ? 1 : 0);
      //Se realiza la petición axios mandando la ruta y el formData
      await axios
        .post("/secciones/", FORM_DATA, {
          headers: {
            Authorization: `Bearer ${token.value}`,
          },
        })
...
async function actualizarSeccion() {
  ...
  if (validarTituloSeccion() && validarSubtituloSeccion() && form.value.id_pagina != 0) {
    try {
      ...
      //Se realiza la petición axios mandando la ruta y el formData
      await axios
        .post("/secciones_update/" + id, FORM_DATA, {
          headers: {
            Authorization: `Bearer ${token.value}`,
          },
        })
