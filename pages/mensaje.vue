...
<template>
    <div class="principal mt-6">
        ...
        <MenuMensajeDashboard class="mr-8" />
        ...
        <div class="mdprincipal flex-col mt-8 px-8 overflow-hidden">
            ...
            <div class="h-16 w-full rounded-xl flex justify-between items-center content-buttons max-[450px]:flex-wrap">
                ...
                <div class="w-3/4 flex items-center h-full mt-4 max-[500px]:w-full">
                    ...
                    <input type="text" class="rounded-lg relative w-2/4 h-12 outline-none max-[800px]:w-full min-w-[200px]"
                        placeholder="Buscar... (asunto/correo)" v-model="buscar.buscador" @keyup="buscarMensajes($event)" />
            ...
            <div class="h-screen">
                ...
                <div class="tables overflow-y-scroll h-3/5 pr-4">
                    <TablaCargando v-if="mensajes.length == 0 && !ceroRegistrosEncontrados" />
                ...
                <div class="flex justify-center mt-6">
                    <Paginacion v-if="mensajes.length > 1 && !ceroRegistrosEncontrados" v-model:pagina_actual="pagina"
                        @cambioDePagina="cambioDePagina" :items_totales="mensajes.length" />
                </div>
            </div>
        </div>
    </div>
...
async function leerMensajes() {
    ...
    try {
        ...
        if ((mensajes.value.length < pagina.value) && pagina.value != 1) {
            //Se actualiza el valor de la constante pagina
            pagina.value = mensajes.value.length;
        }
...
function filtrarPaginas() {
    ...
    data_filtrada.value = data.value.filter(mensaje =>
        mensaje.campos.nombre_contactante.toLowerCase().includes(buscar.value.buscador.toLowerCase()) ||
        mensaje.campos.correo_contactante.toString().includes(buscar.value.buscador)
    );
    ...
    if (data_filtrada.value.length == 0) {
        ...
        for (let i = 0; i < data_filtrada.value.length; i += 10) {
            mensajes.value.push(data_filtrada.value.slice(i, i + 10));
        }
        //Se actualiza el valor de la constante de búsqueda a false
        ceroRegistrosEncontrados.value = false;
    }
