<template>
    <div class="principal mt-6">
        ...
        <div class="mdprincipal flex-col mt-8 px-8 overflow-hidden">
            ...
            <div class="h-16 w-full rounded-xl flex justify-between items-center content-buttons max-[450px]:flex-wrap">
                ...
                <div class="w-3/4 flex items-center h-full mt-4 max-[500px]:w-full">
                    ...
                    <input type="text" class="rounded-lg relative w-2/4 h-12 outline-none max-[800px]:w-full min-w-[200px]"
                        placeholder="Buscar... (nombre rango/cantidad minima)" v-model="buscar.buscador" @keyup="buscarRangos($event)" />
            ...
            <!-- Se manda a traer la longitud del array de rangos (el que trae los registros) y así saber cuantos registros son -->
            <div class="h-screen">
                <p v-if="rangos.length > 0 && !ceroRegistrosEncontrados"
                    class="font-extrabold text-slate-900 mt-8 ml-4 max-[425px]:mt-16">{{ rangos[pagina -
                        1].length
                ...
                <div class="flex-col">
                    <div v-if="rangos.length == 0 && ceroRegistrosEncontrados">
                ...
                <div class="tables overflow-y-scroll h-3/5 pr-4">
                    <TablaCargando v-if="rangos.length == 0 && !ceroRegistrosEncontrados" />
                    <TablesRango v-if="rangos.length > 0" :datos_rangos="rangos" :actualizar_datos="cargarTabla"
                        :paginacion="pagina" />
                </div>
                <div class="flex justify-center mt-6">
                    <Paginacion v-if="rangos.length > 1 && !ceroRegistrosEncontrados" v-model:pagina_actual="pagina"
                        @cambioDePagina="cambioDePagina" :items_totales="rangos.length" />
                </div>
            </div>
        </div>
    </div>
...
onMounted(() => {
    ...
    //Se leen los rangos al montarse la página para evitar problemas del setup y el localStorage
    leerRangos();
});
...
async function leerRangos() {
    ...
    try {
        ...
        if (registros_visibles.value) {
            ...
            const { data: res } = await axios.get('/rangos', {
                headers: {
            ...
            for (let i = 0; i < res.data.length; i += 10) {
                rangos.value.push(res.data.slice(i, i + 10));
            }
            ...
            const { data: res } = await axios.get('/rangos_ocultos', {
                headers: {
                    Authorization: `Bearer ${token.value}`,
                },
            });
        ...
        if ((rangos.value.length < pagina.value) && pagina.value != 1) {
            //Se actualiza el valor de la constante pagina
            pagina.value = rangos.value.length;
        }

        if (rangos.value.length == 0) {
...
function filtrarPaginas() {
    ...
    data_filtrada.value = data.value.filter(rango =>
            rango.campos.nombre_rango.toLowerCase().includes(buscar.value.buscador.toLowerCase())||
            rango.campos.cantidad_monetaria_minima.toLowerCase().includes(buscar.value.buscador.toLowerCase())
            );
    ...
    if (data_filtrada.value.length == 0) {
        ...
        for (let i = 0; i < data_filtrada.value.length; i += 10) {
            rangos.value.push(data_filtrada.value.slice(i, i + 10));
        }
        //Se actualiza el valor de la constante de búsqueda a false
        ceroRegistrosEncontrados.value = false;
    }
