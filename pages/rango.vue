...
<template>
    <div class="principal mt-6">
        <div class="topprincipal flex justify-between font-semibold text-base ml-4">
            <div class="options">
                <NuxtLink to="/donante" class="ml-4">Donantes</NuxtLink>
                <NuxtLink to="" class="active ml-4">Rangos</NuxtLink>
            </div>
...
let rangos = computed(() => data.value.data);
...
async function leerUnRango(id) {
    try {
        ...
        await axios.get('/rangos/' + id).then(res => {
            //Constante para el modal
            const modalElement = document.getElementById('staticModal');
