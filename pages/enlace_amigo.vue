<template>
    <div class="principal mt-6">
        ...
        <MenuEnlaceAmigoDashboard class="mr-8" />
        ...
        <div class="mdprincipal flex-col mt-8 px-8 overflow-hidden">
            ...
            <div class="h-16 w-full rounded-xl flex justify-between items-center content-buttons max-[450px]:flex-wrap">
                ...
                <div class="w-3/4 flex items-center h-full mt-4 max-[500px]:w-full">
                    ...
                    <input type="text" class="rounded-lg relative w-2/4 h-12 outline-none max-[800px]:w-full min-w-[200px]"
                        placeholder="Buscar... (nombre página)" v-model="buscar.buscador" @keyup="buscarEnlaces()" />
            ...
            <!-- Se manda a traer la longitud del array de enlaces (el que trae los registros) y así saber cuantos registros son -->
            <div class="h-screen">
                <p v-if="enlaces.length > 0 && !ceroRegistrosEncontrados"
                    class="font-extrabold text-slate-900 mt-8 ml-4 max-[425px]:mt-16">{{ enlaces[pagina -
                        1].length
                ...
                <div class="flex-col">
                    <div v-if="enlaces.length == 0 && ceroRegistrosEncontrados">
                ...
                <div class="tables overflow-y-scroll h-3/5 pr-4">
                    <TablaCargando v-if="enlaces.lenght==0&&!ceroRegistrosEncontrados"/>
                    <TablesEnlaceAmigo v-if="enlaces.length > 0" :datos_enlaces="enlaces" :actualizar_datos="cargarTabla"
                        :paginacion="pagina" />
                </div>
                <div class="flex justify-center mt-6">
                    <Paginacion v-if="enlaces.length > 1 && !ceroRegistrosEncontrados" v-model:pagina_actual="pagina"
...
onMounted(() => {
    ...
    //Se leen los enlaces al montarse la página para evitar problemas del setup y el localStorage
    leerEnlaces();
});
...
function visibilidadRegistros() {
    ...
    //Se evalua el buscador para realizar leerEnlaces o buscarEnlaces 
    ...
    try {
        if (registros_visibles.value) {
            ...
            for (let i = 0; i < res.data.length; i += 10) {
                enlaces.value.push(res.data.slice(i, i + 10));
            }
            ...
            const { data: res } = await axios.get('/enlaces_amigos_ocultos', {
                headers: {
                    Authorization: `Bearer ${token.value}`,
                },
            });
...
async function buscarEnlaces() {
    try {
        ...
        if (buscar.value.buscador != "") {   
            ...
            data.value = data.value.filter(enlace =>
            ...
            if (data.value.length == 0) {
                ...
                for (let i = 0; i < data.value.length; i += 1) {
                    enlaces.value.push(data.value.slice(i, i + 1));
                }
                //Se actualiza el valor de la constante de búsqueda a false
                ceroRegistrosEncontrados.value = false;
            }
