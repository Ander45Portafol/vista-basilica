        ...
        <div class="mdprincipal flex-col mt-8 px-8 overflow-hidden">
          <!-- Sección del buscador -->
           ...
           <div class="line bg-slate-800 h-0.5 mt-4 w-full min-w-[200px]"></div>
            ...
            <div class="h-screen">
                <p v-if="enlaces" class="font-extrabold text-slate-900 mt-8 ml-4 max-[425px]:mt-16">{{ enlaces.length
                ...
                <div class="tables overflow-y-scroll h-3/5 pr-4">
                    <div v-if="!enlaces" class="loadingtable overflow-hidden h-full pr-4">
                      ...
                      <!-- Se manda a traer la longitud del array de contactos (el que trae los registros) y así saber cuantos registros son -->
                    <TablesEnlaceAmigo v-if="enlaces" :datosEnlace="enlaces" :actualizarDatos="leerEnlaces" />
                </div>
                <div class="flex justify-center mt-6">
                    <TailwindPagination v-if="enlaces"
...
async function leerEnlaces() {
    try {
        ...
        if (registros_visibles.value) {
            ...
            const { data: res } = await axios.get(`/enlaces_amigos_ocultos?page=${pagina.value}`, {
                headers: {
                    Authorization: `Bearer ${token.value}`,
                },
            });
