...
async function buscarDonantes() {
    try {
        ...
        if (buscar.value.buscador != "") {
            ...
            data.value = data.value.filter(donante =>
                donante.campos.nombre_donante.toLowerCase().includes(buscar.value.buscador.toLowerCase()) ||
                donante.campos.correo_donante.toLowerCase().includes(buscar.value.buscador.toLowerCase())
            );
            ...
            if (data.value.length == 0) {
                ...
                for (let i = 0; i < data.value.length; i += 1) {
                    donantes.value.push(data.value.slice(i, i + 1));
                }
                //Se actualiza el valor de la constante de búsqueda a false
                ceroRegistrosEncontrados.value = false;
            }
