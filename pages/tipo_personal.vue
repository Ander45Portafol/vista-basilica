...
onMounted(() => {
    ...
    //Se leen los tipos de personales al montarse la página para evitar problemas del setup y el localStorage
    leerTiposPersonales();
});
    ...
    try {
        if (registros_visibles.value) {
            ...
            for (let i = 0; i < res.data.length; i += 10) {
                tipos_personales.value.push(res.data.slice(i, i + 10));
            }
