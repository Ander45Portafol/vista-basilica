<template>
    <div class="principal mt-6">
        ...
        <div class="mdprincipal flex-col mt-8 px-8 overflow-hidden">
            ...
            <div class="h-16 w-full rounded-xl flex justify-between items-center content-buttons max-[450px]:flex-wrap">
                ...
                <div class="w-3/4 flex items-center h-full mt-4 max-[500px]:w-full">
                    ...
                    <input type="text" class="rounded-lg relative w-2/4 h-12 outline-none max-[800px]:w-full min-w-[200px]"
                        placeholder="Buscar... (nombre componente)" v-model="buscar.buscador" @keyup="buscarComponentes($event)" />
            ...
            <!-- Se manda a traer la longitud del array de componentes (el que trae los registros) y así saber cuantos registros son -->
            <div class="h-screen">
                <p v-if="componentes.length > 0 && !ceroRegistrosEncontrados"
                    class="font-extrabold text-slate-900 mt-8 ml-4 max-[425px]:mt-16">{{ componentes[pagina -
                        1].length
                ...
                <div class="flex-col">
                    <div v-if="componentes.length == 0 && ceroRegistrosEncontrados">
                ...
                <div class="tables overflow-y-scroll h-3/5 pr-4">
                    <TablaCargando v-if="componentes.length == 0 && !ceroRegistrosEncontrados" />
                    <TablesComponentes v-if="componentes.length > 0" :datos_componentes="componentes" :actualizar_datos="cargarTabla"
                        :paginacion="pagina" />
                </div>
                <div class="flex justify-center mt-6">
                    <Paginacion v-if="componentes.length > 1 && !ceroRegistrosEncontrados" v-model:pagina_actual="pagina"
                        @cambioDePagina="cambioDePagina" :items_totales="componentes.length" />
                </div>
            </div>
        </div>
    </div>
...
onMounted(() => {
    ...
    //Se leen los componentes al montarse la página para evitar problemas del setup y el localStorage
    leerComponentes();
});
...
function filtrarPaginas() {
    //Se filtran los registros de data según los parámetros del buscador (nombre_componente )
    const data_filtrada = ref();

    data_filtrada.value = data.value.filter(componente =>
    componente.campos.nombre_componente.toLowerCase().includes(buscar.value.buscador.toLowerCase())
    );
    ...
    if (data_filtrada.value.length == 0) {
        ...
        for (let i = 0; i < data_filtrada.value.length; i += 10) {
            componentes.value.push(data_filtrada.value.slice(i, i + 10));
        }
        //Se actualiza el valor de la constante de búsqueda a false
        ceroRegistrosEncontrados.value = false;
    }
