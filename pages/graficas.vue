<template>
  <div class="mt-4">
    <div class="topprincipal flex justify-between font-semibold text-base ml-4 mr-8">
    ...
    <div
      v-if="
        !data_lista_donaciones &&
        !data_lista_anuncios &&
        !data_lista_n_secciones &&
        !data_lista_usuarios &&
        !data_lista_eventos
      "
    ...
    <div
      v-else
      class="mdprincipal h-screen overflow-y-scroll flex-col mt-6 pl-6 pr-8 py-10"
    >
      <div class="h-2/3 w-full bg-slate-200 rounded-2xl pl-4">
        <div class="flex-row text-center pt-4">
          <!-- Título de la gráfica -->
          <p class="text-2xl font-extrabold mt-5">
            Donaciones registradas por fecha en la semana actual
          </p>
          <p class="text-xl font-bold">
            Total donado:
            <span class="text-xl font-normal" v-if="total_suma_donaciones"
              >${{ total_suma_donaciones }}</span
            ><span class="text-xl font-normal" v-else>$0.00</span>
          </p>
          <div class="grafic h-[45vh] w-full flex justify-center items-center">
            <!-- Se valida si el gráfico esta listo para mostrarse -->
            <Line
              v-if="
                data_donaciones &&
                data_donaciones.results.length > 0 &&
                data_lista_donaciones
              "
              :data="CHART_DONACIONES"
              :options="OPCIONES_DONACIONES"
              class="px-10"
            />
            <!-- Si no esta listo se muestra un mensaje de que no hay información -->
            <p v-else-if="data_lista_donaciones">
              No hay donaciones registradas en esta semana.
            </p>
          </div>
        </div>
      </div>
      <div class="flex w-full justify-between mt-10 max-[1000px]:flex-wrap">
        ...
        >
          ...
          <div class="text-left p-4">
            <p class="text-2xl text-center mt-3 font-bold">
              Cantidad de secciones por página
            </p>
          </div>
          <div
            class="grafic h-3/4 w-full flex justify-center items-center max-[950px]:w-full"
          >
            <!-- Se valida si el gráfico esta listo para mostrarse -->
            <PolarArea
              v-if="
                data_n_secciones &&
                data_n_secciones.results.length > 0 &&
                data_lista_n_secciones
              "
              :data="CHART_N_SECCIONES"
              :options="OPCIONES_N_SECCIONES"
            />
            <!-- Si no esta listo se muestra un mensaje de que no hay información -->
        ...
        >
          ...
          <div class="text-left p-4">
            ...
            <p class="text-xl text-center font-bold">
              Usuarios totales:
              <span class="text-xl font-normal"> {{ total_usuarios }} </span>
            </p>
          </div>
          <div class="grafic h-2/3 w-full flex justify-center items-center">
            ...
            <Pie
              v-if="
                data_n_usuarios &&
                data_n_usuarios.results.length > 0 &&
                data_lista_usuarios
              "
              :data="CHART_N_USUARIOS"
              :options="OPCIONES_N_USUARIOS"
            />
            <!-- Si no esta listo se muestra un mensaje de que no hay información -->
      ...
      <div class="flex w-full justify-between mt-10 mb-[100px] max-[1000px]:flex-wrap">
        ...
        >
          ...
          <div class="text-left p-4">
            <p class="text-2xl text-center mt-3 font-bold">
              Cantidad de eventos por fecha en la semana actual
            </p>
          </div>
          <div class="grafic h-2/3 w-full flex justify-center items-center">
            <!-- Se valida si el gráfico esta listo para mostrarse -->
            <Bar
              v-if="data_eventos && data_eventos.results.length > 0 && data_lista_eventos"
              :data="CHART_EVENTOS"
        ...
        >
          ...
          <div class="text-left p-4">
            <p class="text-2xl text-center mt-3 font-bold">
              Cantidad de anuncios por fecha en el mes actual
            </p>
          </div>
          <div class="grafic h-2/3 w-full flex justify-center items-center">
            <!-- Se valida si el gráfico esta listo para mostrarse -->
...
const CHART_EVENTOS = computed(() => {
  return {
    //Se establecen los labels que son los textos que se muestran abajo del eje x, además se usa el map para evaluar cada registro indivualmente
    labels: data_eventos.value.results.map((item) => item.fecha_evento),
    //Se configuran los dataset de la gráfica que se mostrarán cuando hayan datos
    datasets: [
      {
        //Se le agrega un prefijo personalizado a los labels, que además muestran el valor de la data para ese registro individual
        label: "N° de eventos",
        //Se establece la data de la grafica con un map de los registros
        data: data_eventos.value.results.map((item) => item.cantidad_eventos),
