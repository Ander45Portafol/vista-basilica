    ...
    <div class="principal mt-6">
        <div class="topprincipal flex justify-between font-semibold text-base ml-4">
            <div class="options">
                ...
                <NuxtLink class="active ml-4" to="/grupo_parroquial">Grupos</NuxtLink>
                    ...
                    <div class="flex justify-start w-3/4 items-center max-[400px]:w-full">
                        ...
                        <div
                            ...
                            <p class="font-extrabold text-xl text-salte-900 max-[750px]:text-[18px]">{{
                                categoria_grupo.nombre_categoria_grupo }}</p>
                        </div>
                    </div>
                    <!-- Al darle clic al evento leerUnaCategoriaGrupo ejecuta la funcion -->
                    <div
                        class="buttons-data flex justify-center items-center max-[750px]:flex-col max-[400px]:flex-row max-[400px]:m-auto max-[400px]:mt-2">
                        <button class="h-10 w-10 rounded-md flex items-center justify-center max-[400px]:mx-4 editbtn"
                            id="btnedit" @click="leerUnaCategoriaGrupo(categoria_grupo.id_categoria_grupo_parroquial)"
                            v-if="categoria_grupo.visibilidad_categoria == 1">
                        ...
                        <!-- Al darle clic al evento borrarCategoriaGrupo ejecuta la funcion -->
                        <button
                            class="h-10 w-10 rounded-md flex items-center justify-center ml-4 deletebtn max-[750px]:ml-0 max-[750px]:mt-2 max-[400px]:mt-0 max-[400px]:mx-4"
                            @click="borrarCategoriaGrupo(categoria_grupo.id_categoria_grupo_parroquial)"
                            v-if="categoria_grupo.visibilidad_categoria == 1">
                        ...
                        <!-- Al darle clic al evento recuperarCategoriaGrupo ejecuta la funcion -->
                        <button @click="recuperarCategoriaGrupo(categoria_grupo.id_categoria_grupo_parroquial)"
...
let categoria_grupos = computed(() => data.value.data);
...
watch(pagina, async () => {
    ...
    if (buscar.value.buscador != "") {
        ...
        //Se ejecuta el leer categoria de los grupos para cargar la tabla, usando la constante pagina también se busca la pagina especifica de registros
        leerCategoriaGrupos();
    }
...
async function leerCategoriaGrupos() {
    try {
        /*Se manda la petición axios para leer categorias de los grupos (no se manda la ruta completa por al configuración de axios -> Para mas información vean el axiosPlugin en la carpeta plugins),
        además usando el valor de la constante "pagina" se filtra la pagina de registros que axios va a traer*/
        const { data: res } = await axios.get(`/categorias_grupos?page=${pagina.value}`);
...
async function buscarCategoriaGrupos() {
    try {
        //Se evalua que el buscador no este vacio
        if (buscar.value.buscador != "") {
            // Realiza la petición axios para llamar a la ruta de búsqueda
            const { data: res } = await axios.get(`/categorias_grupos_search?page=${pagina.value}&buscador=${buscar.value.buscador}`);
...
async function crearCategoriaGrupo() {
    try {
        ...
        const formData = {
            nombre_categoria_grupo: form.value.nombre_categoria_grupo,
            visibilidad_categoria: form.value.visibilidad_categoria,
        };

        //Se realiza la petición axios mandando la ruta y el formData
        await axios.post("/categorias_grupos", formData);

        //Se cargan todas las categoria de los grupos y se cierra el modal
        leerCategoriaGrupos();
        document.getElementById('closeModal').click();
    ...
    try {
        ...
        await axios.get('/categorias_grupos/' + id).then(res => {
            ...
            form.value = {
                ...
                nombre_categoria_grupo: res.data.nombre_categoria_grupo,
                //Se convierte a true o false en caso de que devuelva 1 o 0, esto por que el input solo acepta true y false
                visibilidad_categoria: res.data.visibilidad_categoria ? true : false
            }
        })
    } catch (error) {
        console.log(error);
...
async function actualizarCategoriaGrupo() {
    try {
        ...
        //Se realiza la petición axios mandando la ruta y el formData
        await axios.put("/categorias_grupos/" + id, formData);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                //Se cargan todas las  categoria de los grupos
                leerCategoriaGrupos();

                //Se lanza la alerta de éxito
                Toast.fire({
                    icon: 'success',
                    title: 'Categoria ocultada exitosamente'
                })
            } catch (error) {
                console.log(error);
                const mensajeError = error.response.data.message;
...
async function recuperarCategoriaGrupo(id) {
    //Se lanza una alerta de confirmación
    Swal.fire({
        title: 'Confirmación',
        ...
        if (result.isConfirmed) {
            try {
                ...
                Toast.fire({
                    icon: 'success',
                    title: 'Categoria recuperada exitosamente'
                })
            } catch (error) {
                console.log(error);
                const mensajeError = error.response.data.message;
