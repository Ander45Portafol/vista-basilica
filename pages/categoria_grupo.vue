<template>
    <div class="principal mt-6">
        ...
        <div class="mdprincipal flex-col mt-8 px-8 overflow-hidden">
            ...
            <div class="h-screen">
                <p v-if="categoria_grupos.length > 0 && !ceroRegistrosEncontrados"
                    class="font-extrabold text-slate-900 mt-8 ml-4 max-[425px]:mt-16">{{ categoria_grupos[pagina -
                        1].length
...
onMounted(() => {
    ...
    //Se leen las categorias al montarse la página para evitar problemas del setup y el localStorage
    leerCategoriaGrupos();
});
...
async function leerCategoriaGrupos() {
    ...
    try {
        ...
        if (registros_visibles.value) {
            ...
            const { data: res } = await axios.get('/categorias_grupos', {
                headers: {
        ...
        if (categoria_grupos.value.length == 0) {
...
function buscarCategoriaGrupos(event) {
    try {
