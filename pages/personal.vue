            ...
            <div class="h-16 w-full rounded-xl flex justify-between items-center content-buttons max-[450px]:flex-wrap">
                <div action="" class="w-3/4 flex items-center h-full mt-4 max-[500px]:w-full">
                    ...
                    <input type="text" class="rounded-lg relative w-2/4 h-12 outline-none max-[800px]:w-full min-w-[200px]"
                        placeholder="Buscar... (título/subtítulo)" v-model="buscar.buscador" @keyup="buscarPersonales()">
...
async function llenarSelectPersonales() {
    try {
...
async function buscarPersonales() {
    try {
...
async function leerUnPersonal(id) {
    try {
        ...
        await axios.get('/personal/' + id).then(res => {
            //Constante para el modal
            const modalElement = document.getElementById('staticModal');
...
async function recuperarPersonal(id) {
    //Se lanza una alerta de confirmación
    Swal.fire({
        title: 'Confirmación',
